...
int compare_zoj(const char *file1, const char *file2) {
	...
	if (!f1 || !f2) {
		...
		for (;;) {
			...
			for (;;) {
				...
				while ((!isspace(c1) && c1) || (!isspace(c2) && c2)) { // 只要有一个是空格就离开本层循环
					//printf("loop\n");
					if (c1 == EOF && c2 == EOF) {
						//printf("flag1\n");
						goto end;
					...
					if (c1 == EOF||c2 == EOF){
						if (ispunct(c2)||ispunct(c1)) UEflg = 1;
						//break;
					}
					int UEnow = 0;
					if (c1 != c2) {
	...
	if(ret != OJ_WA && UEflg)
		ret = OJ_UE;
	/* END */
	if (ret == OJ_WA||ret==OJ_PE||ret==OJ_UE){
		if(full_diff)
...
int compile_standard(int lang,char * work_dir) {  // 执行编译操作。
	int pid;
	...
	if (pid == 0) {// 子进程负责编译
		...
		if(lang != 3 && lang != 9 && lang != 6 && lang != 11){//除java/C#/python/freebasic之外的语言执行如下内容
...
int compile_custom(int lang, char*work_dir, char*p_dir){
	// 通过自定义的makefile来编译.
	//下面获取makefile。
	char source_makefile_path[BUFFER_SIZE];
	char goal_makefile_path[BUFFER_SIZE];
	sprintf(goal_makefile_path, "%smakefile", work_dir);
	sprintf(source_makefile_path, "%smakefile_%d", p_dir, lang);
	execute_cmd("/bin/cp '%s' '%s'", source_makefile_path, goal_makefile_path);
	int pid;
	pid = fork();
	if(pid==0){//子进程负责编译。
		struct rlimit LIM;
		...
		// 下面执行编译。
		execute_cmd("/usr/bin/make 2> ce.txt");
		if (DEBUG)
			printf("compile end!\n");
		exit(0);
	}else{
		...
		waitpid(pid, &status, 0);
		//if (lang > 3 && lang < 7)
		status = get_file_size("ce.txt");//启用make时子进程不会捕捉status的，因此必须全部通过ce的文件大小来做判断。
		if (DEBUG)
			printf("status=%d\n", status);
		execute_cmd("/bin/umount bin usr lib lib64 etc/alternatives proc dev 2>&1 >/dev/null");
 		execute_cmd("/bin/umount %s/* 2>&1 >/dev/null",work_dir);
...
int compile(int lang, char*work_dir, char *p_dir){
	char makefile_path[BUFFER_SIZE];
	sprintf(makefile_path, "%smakefile_%d", p_dir, lang);
	printf("Ready to check makefile.\n");
	FILE *fp_makefile = fopen(makefile_path, "r");
	if(fp_makefile!=NULL){
		if(DEBUG) printf("Custom makefile compile.\n");
		fclose(fp_makefile);
		return compile_custom(lang, work_dir, p_dir);
	}else{
		if(DEBUG) printf("Standard makefile compile.\n");
		return compile_standard(lang, work_dir);
	}
...
#ifdef _mysql_h
void create_folder(char*work_dir, const char*path){
	printf("Path=%s\n", path);
	//分析path，并决定是否需要创建目录结构。
	char folder[BUFFER_SIZE];
	char parent[BUFFER_SIZE];
	int begin=0;
	bool first=true;
	for(int i=0;path[i]!='\0';++i){
		if(path[i]=='/'||path[i]=='\\'){//每当找到一个路径分隔符，都从上一个符号开始写入folder
			if(begin<i){
				if(begin-1>=0)folder[begin-1]='/';
				for(int j=begin;j<i;++j){
					folder[j]=path[j];
					if(first)parent[j]=path[j];
				}
				folder[i]='\0';
				execute_cmd("mkdir '%s%s'", work_dir, folder);
				first=false;
				begin = i+1;
			}
		}
	}
	execute_cmd("chmod 777 '%s%s' -R", work_dir, parent);
}

void _get_solution_mysql(int solution_id, char * work_dir, int lang, char *p_dir) {
	...
	//按照修改规则，在这里，通过source_code抓取到了源代码，然后需要结合template文件将源代码转换为源代码文件表。
	char template_path[BUFFER_SIZE];
	sprintf(template_path, "%stemplate_%d", p_dir, lang);
	FILE *fp_template = fopen(template_path, "r");
	if(fp_template!=NULL){ // 表示模板存在。需要解析模板。
		if(DEBUG) printf("Template source.\n");
		std::ifstream ifs;
    	ifs.open(template_path);
    	if(ifs.is_open()){
			Reader reader;
    		Value root;
    		if (reader.parse(ifs, root, false))
    		{
				//解析source_code到json状态。
				Reader source_reader;
				Value source_root;
				if(source_reader.parse(row[0], source_root)){
					Value files = root["file"];
    				int size = files.size();
					for(int i=0;i<size;++i){// 遍历文件列表中的每一个文件。
						Value file = files[i];
						std::string filename = file["filename"].asString();
						Value codes = file["code"];

						sprintf(src_pth, "%s%s", work_dir, filename.c_str());	
						create_folder(work_dir, filename.c_str());
						FILE* fp_src = fopen(src_pth, "we");

						if(fp_src!=NULL){
							int size2 = codes.size();
							for(int j=0;j<size2;++j){// 遍历该文件下的每一个code组件。
								std::string type = codes[j]["type"].asString();
								std::string content = codes[j]["content"].asString();
								if(type=="block"){
									if(!source_root[content].isNull()){
										std::string source_code = source_root[content].asString();
										fprintf(fp_src, "%s", source_code.c_str());
									}
								}else{
									fprintf(fp_src, "%s", content.c_str());
								}
							}
							fclose(fp_src);
						}
...
#endif
void _get_solution_http(int solution_id, char * work_dir, int lang, char *p_dir) {
...
void get_solution(int solution_id, char * work_dir, int lang, char *p_dir) {
	if (http_judge) {
		_get_solution_http(solution_id, work_dir, lang, p_dir);
	} else {
#ifdef _mysql_h
		_get_solution_mysql(solution_id, work_dir, lang, p_dir);
#endif
	}
...
void run_solution(int & lang, char * work_dir, int & time_lmt, int & usedtime,
	...
	case 14://clang++
	case 17://go
		execl("./Main", "./Main", (char *) NULL); // 上述语言都是直接执行可执行文件
		break;
	case 3://java
...
void judge_solution(int & ACflg, int & usedtime, int time_lmt, int isspj,
		int p_id, char * infile, char * outfile, char * userfile, int & PEflg, int & UEflg,
...
int main(int argc, char** argv) {
	...
	*/
	...
	init_parameters(argc, argv, solution_id, runner_id); // 提取启动参数
	...
	sprintf(work_dir, "%s/run%s/", oj_home, argv[2]); // 根据clientid生成run作为工作文件夹。

	clean_workdir(work_dir); // 清理工作文件夹
	if (shm_run)
		mk_shm_workdir(work_dir);

	chdir(work_dir); // 切换工作路径到run文件夹
	
	if (http_judge)
		system("/bin/ln -s ../cookie ./");
	get_solution_info(solution_id, p_id, user_id, lang); // 抓取solution的信息。solutionid=>problemid,userid,language
	//get the limit

	if (p_id == 0) { // 如果找不到题目id，那么按照默认进行设定。
		time_lmt = 5;
		mem_lmt = 128;
		isspj = 0;
	} else {
		get_problem_info(p_id, time_lmt, mem_lmt, isspj); // 抓取problem的信息。problemid=>timelimit, memorylimit, isspecialjudge
	}
	//copy source file
	sprintf(p_dir, "%s/data/%d/", oj_home, p_id);
	get_solution(solution_id, work_dir, lang, p_dir); // 获得solution,同时也将数据写入到run路径下。
	//java is lucky
	//等等，这里这些lang都是个什么意思？
	if (lang >= 3 && lang != 10 && lang != 13 && lang != 14) {  // Clang Clang++ not VM or Script
	//如果lang是：java/ruby/bash/python/php/perl/bas/scm/lua/js/go
		// the limit for java
		time_lmt = time_lmt + java_time_bonus;  // 这些语言要额外加上时间和空间宽限。
		mem_lmt = mem_lmt + java_memory_bonus;  // fix 也许对于sdustoj，应该去掉这个宽限？
		// copy java.policy
		if(lang==3){//对于java，将限制文件从etc拷贝过来。
			execute_cmd("/bin/cp %s/etc/java0.policy %s/java.policy", oj_home,work_dir);
			execute_cmd("chmod 755 %s/java.policy", work_dir);//给它rwxr-xr-x的权限
			execute_cmd("chown judge %s/java.policy", work_dir);//更改owner
		}
	...
	Compile_OK = compile(lang,work_dir, p_dir); // 执行编译。参数是language，工作路径。
	if (Compile_OK != 0) {  // 编译错误
		addceinfo(solution_id); // 添加编译信息？
		update_solution(solution_id, OJ_CE, 0, 0, 0, 0, 0.0); // 更新提交状态。更新为Compile error
		update_user(user_id);  // 更新user
		update_problem(p_id);  // 更新题目
#ifdef _mysql_h
		if (!http_judge)
			mysql_close(conn);
#endif
		clean_workdir(work_dir);  // 清理工作目录
		write_log("compile error");
		exit(0); // 退出client
	} else {
		
		update_solution(solution_id, OJ_RI, 0, 0, 0, 0, 0.0); // 更新提交的状态为Running Judge
		umount(work_dir); // 看起来像是卸载了某些设备。安全作用？
	...
	//将fullpath赋值为保存着problem的全部信息的文件夹路径
	...
	if (p_id > 0 && (dp = opendir(fullpath)) == NULL) { // 这里打开dir？
	...
	int ACflg, PEflg, UEflg;
	ACflg = PEflg = UEflg = OJ_AC;
	int namelen;
	int usedtime = 0, topmemory = 0;

	//create chroot for ruby bash python
	//下面是为了将这些语言的运行环境拷贝到工作目录下。
	//等等，所以你才判一次题费了死事？
	if (lang == 4)
	...
	if (p_id == 0) {  // 执行没有题目的自定义输入
		printf("running a custom input...\n");
		get_custominput(solution_id, work_dir);  // 获得自定义输入
		init_syscalls_limits(lang); // 初始化系统调用限制。
		pid_t pidApp = fork(); // 又fork了一下，不知道要干什么。

		if (pidApp == 0) {// 子进程负责run
		...
		if (ACflg == OJ_TL) { // 如果超时，那么将一用时间定成时间限制×1k
			usedtime = time_lmt * 1000;
		}
		if (ACflg == OJ_RE) { // 如果是runtime error，就追加RE信息。
			if (DEBUG)
		...
		update_solution(solution_id, OJ_TR, usedtime, topmemory >> 10, 0, 0, 0); // 更新提交信息
		clean_workdir(work_dir);
		exit(0); // 离开
	}
	// 如果上面的内容没有执行，说明不是自定义输入内容，是一个题目
	for (; (oi_mode || ACflg == OJ_AC|| ACflg == OJ_PE||ACflg == OJ_UE) && (dirp = readdir(dp)) != NULL;) {//dp文件夹是包含题目资料的文件夹路径
		// 无限循环，直到：oi模式或者AC/PE/UE了，并且reddir非null。这里是遍历目录下拥有的所有文件。

		namelen = isInFile(dirp->d_name); // check if the file is *.in or not
		//只有在当前文件是in文件时才继续下面的操作。
		if (namelen == 0)
			continue;
		...
		prepare_files(dirp->d_name, namelen, infile, p_id, work_dir, outfile, // 准备文件？
				userfile, runner_id);
		init_syscalls_limits(lang); // 初始化系统调用限制？
		...
		if (pidApp == 0) { // 还是子进程跑
			...
			judge_solution(ACflg, usedtime, time_lmt, isspj, p_id, infile,
					outfile, userfile, PEflg, UEflg, lang, work_dir, topmemory,
	...
	if (ACflg == OJ_AC && PEflg == OJ_PE)
		ACflg = OJ_PE; //PE标记
	if (ACflg == OJ_AC && UEflg == OJ_UE)
		ACflg = OJ_UE;
	if (sim_enable && ACflg == OJ_AC && (!oi_mode || finalACflg == OJ_AC) // 我也不知道这是什么
	...
	if (ACflg == OJ_TL) { // 超时
		usedtime = time_lmt * 1000; // 还是时限×1k
	}
	if (oi_mode) {
		if (num_of_test > 0)
